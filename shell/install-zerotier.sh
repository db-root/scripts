...
# >>> MODIFIED: Use NJU mirror <<<
ZT_BASE_URL_HTTPS='https://mirrors.nju.edu.cn/zerotier/'
ZT_BASE_URL_HTTP='http://mirrors.nju.edu.cn/zerotier/'
...
echo '***   Anolis OS (DragonFly Linux)'
echo
...
# >>> MODIFIED: Add alinux (Alibaba Cloud Linux) and anolis (Anolis OS) <<<
elif [ $ID == "centos" ] || [ $ID == "rocky" ] || [ $ID == "almalinux" ] || [ $ID == "rhel" ] || [ $ID == "fedora" ] || [ $ID == "amzn" ] || [ $ID == "sangoma" ] || [ $ID == "ol" ] || [ $ID == "alinux" ] || [ $ID == "anolis" ]; then
    ...
    # >>> NEW: Handle Alibaba Cloud Linux 3 and Anolis OS <<<
    elif [ "$ID" == "alinux" ] && [ "$VERSION_ID" == "3" ]; then
        echo "*** Found Alibaba Cloud Linux 3, using el/8 repo"
        baseurl="${ZT_BASE_URL_HTTP}redhat/el/8"
    elif [ "$ID" == "anolis" ]; then
        if [ "${VERSION_ID%%.*}" -ge 23 ]; then
            echo "*** Found Anolis OS 23+, using el/9 repo"
            baseurl="${ZT_BASE_URL_HTTP}redhat/el/9"
        else
            echo "*** Found Anolis OS 8, using el/8 repo"
            baseurl="${ZT_BASE_URL_HTTP}redhat/el/8"
        fi
    else
